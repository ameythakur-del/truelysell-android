    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        rgPayType.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup radioGroup, int checkedId) {
                ...
                if (checkedId == rbCOD.getId()) {
                    type = "1";
                } else if (checkedId == rbWallet.getId()) {
                    type = "2";
                }
            }
        });
            ...
            case R.id.btn_booknow:
                ...
                if (type.isEmpty()) {
                    Toast.makeText(mActivity,
                            "Select any one payment option", Toast.LENGTH_SHORT).show();
                } else {
...
//                    }
                                ...
                                mActivity.generateCardPayment(type);
                            }
                        }
                    } else {

                        ((ActivityBookService) mActivity).bookServiceDetails.setNotes(etDescription.getText().toString().trim());
                        if (((ActivityBookService) mActivity).bookServiceDetails.getFromTime().isEmpty() && ((ActivityBookService) mActivity).bookServiceDetails.getToTime().isEmpty()) {
                            Toast.makeText(mActivity, "Choose TimeSlots", Toast.LENGTH_SHORT).show();
                        } else if (((ActivityBookService) mActivity).bookServiceDetails.getLocation().isEmpty()) {
                            Toast.makeText(mActivity, "Choose your location", Toast.LENGTH_SHORT).show();
                        } else if (((ActivityBookService) mActivity).bookServiceDetails.getNotes().isEmpty()) {
                            AppUtils.showToast(getActivity(), getString(R.string.err_txt_desc_empty));
                        } else {
                            ((ActivityBookService) mActivity).generateCardPayment(type);
                        }
